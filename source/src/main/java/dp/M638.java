...
public class M638 {
    ...
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        ...
        for (int i = 0; i < special.size(); i++) {
            List<Integer> pck = special.get(i);
            if (!check(pck, needs)) {
                continue;
            }
            ...
            for (int j = 0; j < price.size(); j++) {
                remains.add(needs.get(j) - pck.get(j));
            }

            int pckPrice = pck.get(price.size());;
            sum = Math.min(sum, pckPrice + shoppingOffers(price, special, remains));
        }

        return sum;
    }

    public boolean check(List<Integer> special, List<Integer> needs) {
